...
const GECKO = process.env.GECKODRIVER_BIN || process.env.GECKO || `${process.cwd()}/node_modules/.bin/geckodriver`;
const FENIX_PACKAGE = process.env.FENIX_PACKAGE || 'org.mozilla.firefox';
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const SESSION_RETRY_ERROR = /^Could not launch Android [\w.]+\/org\.mozilla\.fenix\.IntentReceiverActivity: Resource temporarily unavailable \(os error 11\)$/;
...
async function startAndroidSession(report) {
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    try {
      return await new Builder().forBrowser('firefox').setFirefoxOptions(firefoxOptions()).setFirefoxService(new ServiceBuilder(GECKO).addArguments('--android-storage', 'internal')).build();
    } catch (error) {
      if (attempt === 3 || !SESSION_RETRY_ERROR.test(String(error?.message ?? error))) throw error;
      await sleep(2_000);
    }
  }
...
async function snapshot(driver) {
  return driver.executeScript(function () {
    ...
    return { marker: document.documentElement.dataset.ytaBench || null, status: document.documentElement.dataset.ytaStatus || null, reason: document.documentElement.dataset.ytaReason || null, currentSrc: currentSrc.slice(0, 160), hijacked: currentSrc.includes('/videoplayback'), readyState: video?.readyState ?? null };
  });
}
...
async function navigateUntilContentScriptAttached(driver, fixtureUrl, report) {
  ...
  throw new Error(`fixture content script did not attach after 3 navigations: ${JSON.stringify(last)}`);
}
...
const report = { xpi: XPI, fenixPackage: FENIX_PACKAGE, geckodriver: GECKO, sessionAttempts: 0, fixtureNavigations: 0, fixtureOrigin: null, addonId: null, snapshot: null, playerRequests: 0, consent: null, extension: null, verdict: 'FAIL' };
...
try {
  ...
  report.extension = await driver.executeScript(function () {
    ...
    return { id: browser.runtime.id, contentScriptMatches: manifest.content_scripts?.map((contentScript) => contentScript.matches) ?? [] };
  });
  ...
  report.playerRequests = fixture.getRequests().filter((request) => request.method === 'POST' && request.path === '/youtubei/v1/player').length;
  report.verdict = report.snapshot.status === 'active' && report.snapshot.hijacked && report.playerRequests >= 1 ? 'PASS' : 'FAIL';
} catch (error) {
