...
- Blocking Android CI runs a hermetic Fenix matrix from the minimum supported Fenix 128 through current, including both sides of the Android 141/142 boundary. Its BENCH-only build permits the emulator host alias `10.0.2.2`, while production host permissions remain unchanged. The probe binds the local fixture on the runner, then establishes an `adb reverse` map for its ephemeral port and loads it through emulator `localhost`. This preserves a secure context for the nonce-authenticated content-to-MAIN bridge and advertises media at the BENCH-only loopback origin accepted by the existing media safety policy, without changing production extension code or network permissions. Before any playback assertion, it proves the loaded fixture origin is secure and exposes `crypto.randomUUID`. The emulator runner starts the host adb daemon before launch and invokes one checked-in shell script so state shared between setup commands cannot be lost across the action's per-line shells. Geckodriver first establishes an Android Marionette session, which verifies Fenix is ready and completes app-data preparation. The probe then drives Fenix's native Remote debugging setting through a uiautomator helper that waits for a valid hierarchy rather than an elapsed launch delay, retries command failures, missing dumps, malformed XML, and the complete menu-to-Settings route with backoff, accepts the menu layouts used across supported releases, and reports the raw failed commands and dump. It waits for the resulting RDP socket, retrying transient adb failures and reporting the final command output if the socket never appears, then uploads and temporarily installs the XPI through Firefox Android's RDP add-ons actor. This remains necessary because Fenix 128 rejects Marionette's temporary-add-on command and later tested releases did not load its content script after that command. The live-YouTube Android playback probe remains a separate non-gating nightly canary.
