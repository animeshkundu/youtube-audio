...
- A device-gated Fenix probe (manual, not a CI gate) exercises the real suspended-rAF path on Firefox for Android: a cold `m.youtube.com/watch` load must reach `active` and the extension must re-hijack the native element replacement, holding the `/videoplayback` source on the swapped-in element. Audio-decode hold (sustained audible playback) requires a real device, because the headless emulator cannot supply the trusted user gesture YouTube's player gate demands.
