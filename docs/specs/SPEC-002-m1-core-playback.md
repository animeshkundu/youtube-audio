...
- A Fenix probe (manual, not a CI gate) exercises the real suspended-rAF path on Firefox for Android: a cold `m.youtube.com/watch` load must reach `active` and the extension must re-hijack the native element replacement, holding the `/videoplayback` source on the swapped-in element. Sustained audible playback is validatable on a **headful** emulator (the earlier "physical device required" claim was wrong): the trusted user gesture only failed because a fresh geckodriver profile floats Fenix's add-on confirmation and onboarding over the Gecko content (`innerWidth`/`innerHeight` 0, `document.hasFocus()` false), so taps never reached the page. After dismissing those overlays, foregrounding the watch URL (`adb shell am start -W -a android.intent.action.VIEW -d <url> org.mozilla.fenix`), waiting for uiautomator to expose the Play control, and `adb shell input tap`, `navigator.userActivation.hasBeenActive` becomes true and audio-only playback holds (`readyState 4`, unmuted, advancing `currentTime`, `/videoplayback` throughout, never the native blob). One caveat observed under real playback that source-only probes miss: around 35-40 s the stream re-sources to a different googlevideo host with a brief clock reset, then resumes and keeps holding audio-only (tracked as a follow-up).
