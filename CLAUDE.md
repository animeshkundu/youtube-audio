...
- Keep exactly four production **content-script** matches for YouTube; never widen content-script matching to `*://*/*`. The credentialless fetch-origin permissions for `*://*.googlevideo.com/*` and `https://sponsor.ajay.app/*` are separate, intended, and required. SponsorBlock traffic requires both data consent and explicit opt-in.
...
│                           #   sponsorblock, telemetry, quality-of-life, download, config, spa, platform,
│                           #   logger + redact + report + diagnostics (PII-free local log)
├── tests/
